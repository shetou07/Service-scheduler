...
describe('SchedulingService bulk availability', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    db.service.findUnique = jest
      .fn()
      .mockResolvedValue({ id: 'service-1', status: ServiceStatus.ACTIVE });
  });

  it('expands selected weekdays and reports overlapping dates as skipped', async () => {
    db.availability.findMany = jest
      .fn()
      .mockResolvedValue([
    ...
    expect(result.skipped).toEqual([
      { date: '2026-09-09', reason: 'Overlaps existing availability for this service' },
    ]);
