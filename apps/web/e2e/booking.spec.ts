...
test('client can progress through an API-backed booking flow', async ({ page }) => {
      ...
      json: [
        {
          ...
          description: 'Focused coaching session.',
          durationMinutes: 60,
          priceMinor: 150000,
        },
      ],
  ...
  await page.route('**/api/services/pt/available-dates', (route) =>
    route.fulfill({ json: [{ date: '2027-01-10T00:00:00.000Z' }] }),
  );
  await page.route('**/api/services/pt/slots?date=2027-01-10', (route) =>
    route.fulfill({
      json: [
        {
          id: 'slot-1',
          startAt: '2027-01-10T07:00:00.000Z',
          endAt: '2027-01-10T08:00:00.000Z',
          capacity: 1,
          bookedCount: 0,
        },
      ],
  ...
  await page.route('**/api/bookings', (route) =>
    route.fulfill({ json: { bookingReference: 'CR-20270110-10000' } }),
  );
