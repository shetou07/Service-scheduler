...
export default function Services() {
  ...
  async function createService(event: FormEvent) {
    ...
    setError('');
    try {
      await adminFetch('/admin/services', { method: 'POST', body: JSON.stringify(form) });
      setForm(emptyForm);
      await load();
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : 'Unable to create service');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function updateService(event: FormEvent) {
    ...
    try {
      await adminFetch(`/admin/services/${id}`, { method: 'PATCH', body: JSON.stringify(changes) });
      setEditing(null);
      await load();
    } catch (reason) {
  ...
  async function deleteService(service: Service) {
    if (
      !window.confirm(
        `Delete ${service.name}? It will be removed from public booking, but existing booking history will be kept.`,
      )
    )
      return;
    setSaving(true);
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : 'Unable to delete service');
    } finally {
      setSaving(false);
    }
  }
        ...
        <form onSubmit={createService}>
          <label>
            ...
            <input
              ...
              required
              minLength={2}
              placeholder="e.g. Speed development"
              value={form.name}
          ...
          <label>
            ...
            <input
              ...
              required
              minLength={5}
              placeholder="Brief description shown to athletes"
          ...
          <label>
            ...
            <input
              ...
              onChange={(event) => setForm({ ...form, priceMinor: Number(event.target.value) })}
            />
          </label>
      ...
      {editing && (
        <section className="data-panel cut-corner">
          <div className="admin-header">
            <div>
              <div className="eyebrow">Editing catalogue item</div>
          ...
          <form onSubmit={updateService}>
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) =>
                  setEditing({ ...editing, durationMinutes: Number(event.target.value) })
                }
              />
            </label>
            <label>
              ...
              <input
                ...
                onChange={(event) =>
                  setEditing({ ...editing, priceMinor: Number(event.target.value) })
                }
              />
            </label>
            <label>
              ...
              <select
                ...
                onChange={(event) =>
                  setEditing({ ...editing, status: event.target.value as ServiceStatus })
                }
              >
                <option value="ACTIVE">Active — visible to clients</option>
                <option value="INACTIVE">Inactive — hidden from clients</option>
                <option value="ARCHIVED">Archived — retained for history</option>
              </select>
            </label>
          ...
          <tbody>
            {items.map((service) => (
              <tr key={service.id}>
                ...
                <td>UGX {service.priceMinor.toLocaleString()}</td>
                <td>
